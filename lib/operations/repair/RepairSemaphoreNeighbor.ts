...
export class RepairSemaphoreNeighbor extends TransformationOperation {
...
`,
      'repair semaphore neighbor',
    );
  }

  protected _transform(bindings: Bindings): void {
    const route2 = this.getSafe(bindings, 'route2');
    const semaphore = this.getSafe(bindings, 'semaphore');
